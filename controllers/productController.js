...
const {generateProductDescription,generateProductTags} = require("../services/aiService");
...
async function getAIProductTags(req, res,next) {
  try {
    const product = await Product.findById(req.params.id);

    if (!product)return next(new AppError('product not found',404));
    const aiTags = await generateProductTags(product);
 product.tags=aiTags;
 await product.save()
    res.status(200).json({
      productId: product._id,
...
module.exports ={uploadProduct,updateProduct,getProducts,
  ...
  stockAlert,getAIProductDescription,
  getAIProductTags,rankedSearch
};